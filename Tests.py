...
def test_T05_prediction(
    ...
    *,
    ...
    passed = met["energy_frac"] >= energy_frac_min and met["norm_err"] <= norm_err_max
    return _result("T-05", passed, met,
        {"n": int(len(df)), "filter": filter_expr,
         "targets": {"energy_frac_min": energy_frac_min, "norm_err_max": norm_err_max}})
...
def save_excel_report(
    ...
    seuils_row = [
        ...
        "—", f"≥ {thresholds['energy_frac_min']:.0%}", f"≤ {thresholds['norm_err_max']:.0%}",
        "—",
    ]
    ...
    ws["A1"] = (
        ...
        f"Seuil ON : P_reel > {thresholds['binarisation_threshold']} W"
    )
...
if __name__ == "__main__":
    ...
    THRESHOLDS = {
        ...
        "f1_min":                 0.70,
        "fpr_max":                0.15,
        "energy_frac_min":        0.85,
        "norm_err_max":           0.15,
        "binarisation_threshold": 0.15,   # seuil P_reel > X → ON
    }
    # ──────────────────────────────────────────────────────────────────────────
