...
OPTI_DIR    = Path(__file__).parent                          # dossier opti/
ENERGY_FILE = OPTI_DIR / "austin_processed_energy_data.csv" # fichier dans opti/
...
def charger_journee(dataid=None, date=None):
    ...
    """
    df = pd.read_csv(
    r"C:\Users\Samia\OneDrive - polymtlus\Bureau\DataThermo\yanis\austin_processed_energy_data.csv", parse_dates=["local_15min"])
    ...
    if date is None:
        date_choisie = pd.to_datetime("2018-07-05").date()
    else:
...
def construire_donnees_modele(df_jour, params):
    ...
    # Temperature exterieure — detection automatique du nom de colonne
